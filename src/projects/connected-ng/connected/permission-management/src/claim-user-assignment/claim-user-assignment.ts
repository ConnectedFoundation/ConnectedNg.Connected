...
import { UserService, UserSelectList, User } from '@connected-ng/connected/users';
	...
	private async load(claimValue: string) {
		...
		try {
			const [users, claims] = await Promise.all([
				...
				firstValueFrom(this.claimService.query({ schemas: ['user'], entities: [CLAIM_UNDEFINED], entityIds: [CLAIM_UNDEFINED] }))
			]);
			this.allUsers = users ?? [];
