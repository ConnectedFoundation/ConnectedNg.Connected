...
import { Claim, ClaimService, CLAIM_UNDEFINED } from '@connected-ng/connected/permissions';
import { ClaimSelectList } from '@connected-ng/connected/permissions';
import { UserService } from '@connected-ng/connected/users';
	...
	private async load(userId: number) {
		...
		try {
			...
			const claims = await firstValueFrom(
				this.claimService.query({ schemas: ['user'], identities: [user.token], entities: [CLAIM_UNDEFINED], entityIds: [CLAIM_UNDEFINED] })
			);
